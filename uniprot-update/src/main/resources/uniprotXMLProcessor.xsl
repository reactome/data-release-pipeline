	...
	<xsl:template match="uniprot:feature[@type='initiator methionine']">
		<xsl:element name="chain">
			...
			<xsl:attribute name="position">
				<xsl:value-of select="./uniprot:location/uniprot:position/@position"/>
			</xsl:attribute>
		</xsl:element>
	</xsl:template>
		...
		<xsl:if test=" contains($featureTypes, concat('|', @type, '|') ) ">
			<xsl:element name="chain">
				...
				<xsl:attribute name="begin">
					<xsl:value-of select="./uniprot:location/uniprot:begin/@position"/>
				</xsl:attribute>
				<xsl:attribute name="end">
					<xsl:value-of select="./uniprot:location/uniprot:end/@position"/>
				</xsl:attribute>
			</xsl:element>
		</xsl:if>
	</xsl:template>
